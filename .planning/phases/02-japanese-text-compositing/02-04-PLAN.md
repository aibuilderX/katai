...
**Scope justification (2 tasks, 5 files):** Task 1 (orchestrator + storage) creates the compositeCampaignImages function. Task 2 (pipeline integration + UI) wires it in. These are kept combined because: (1) Task 2 is untestable without Task 1 -- splitting into separate plans would require Task 2 to blindly import a function from a prior plan, defeating verification. (2) The 5 files break down as 2 files in Task 1 (orchestrator + schema update) and 3 files in Task 2 (two API routes + one component), each well within the 3-5 file guideline. (3) The "7 integration points" are import calls within a single orchestrator function, not separate subsystem integrations -- they're all consumed in one pipeline flow. Splitting would add overhead without improving quality.
</objective>
  ...
  <action>
    **src/lib/compositing/index.ts:**
    ...
    2. Error handling:
       ...
       - **Core module failure handling:**
         - If `analyzeImageLayout()` fails (Claude Vision error, timeout), use fallback layouts from layout-engine.ts (the module should export a `getDefaultLayouts(width, height)` function returning 3 standard positions: top-left horizontal, center horizontal, right-side vertical)
         - If `breakJapaneseText()` throws on malformed text input, catch and use the unsegmented text as a single line (no line breaks applied)
         - If `buildTextSvg()` or `buildVerticalTextSvg()` throws, catch and skip that text element from the composite (log warning with element name)
         - If `buildCtaSvg()` throws, catch and skip CTA from the composite
         - If ALL text elements fail for a single image, skip that image entirely and continue batch processing (log error with image asset ID)
         - If `analyzeRegionContrast()` fails, fall back to white text with drop shadow (safe default for most ad images)
...
<task type="auto">
  ...
  <action>
    **ASYNC PIPELINE NOTE (per discretion area on compositing timing):**
    The existing `runDirectGeneration()` in campaigns/route.ts already runs asynchronously -- the POST handler returns an immediate 201 response to the client, then generation continues in the background. Compositing is added as a new stage WITHIN this existing async pipeline, NOT as a synchronous blocking call on the API response. The user sees progress via Supabase Realtime updates (established pattern from Phase 1).

    **Performance justification for async-within-pipeline:**
    - Claude Vision layout analysis: ~2-5s per image
    - Sharp compositing: ~100ms per composite
    - 4 images x 3 layouts = 12 composites total
    - Estimated total compositing time: ~15-30s (acceptable within the existing async flow that already takes 30-60s for copy + image generation)
    - Progress updates via Supabase Realtime keep the user informed of compositing stage (already established pattern)

    **src/app/api/campaigns/route.ts -- update runDirectGeneration():**

    Add compositing as a new stage after image generation, before marking campaign complete (all within the existing async pipeline -- the API response was already returned to the client):
