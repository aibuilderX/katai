...
- Log errors to console for debugging
  ...
  <verify>
    ...
    - GENX-08: Verify circuit breaker is active for image generation. Run `grep -r "providerHealth\|ProviderHealth" src/lib/ai/flux.ts`. If NO matches found: providerHealth is NOT integrated in flux.ts yet (only in video-pipeline.ts). In that case, add providerHealth check in the generate-images endpoint before calling generateCampaignImages — import ProviderHealthTracker from provider-health.ts, check fal_ai provider health before each generation call, record success/failure after each call. This ensures GENX-08 circuit breaker covers image generation (not just video).
  </verify>
  <done>
    ...
    - GENX-08: Circuit breaker protects image generation via providerHealth check in generate-images endpoint
  </done>
</task>
...
- GENX-08: `grep -r "providerHealth\|ProviderHealth" src/app/api/internal/generate-images/route.ts` returns matches (circuit breaker active for Flux generation)
</verification>
