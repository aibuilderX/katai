...
must_haves:
  truths:
    ...
    - "/api/internal/build-prompt endpoint serializes Phase 9.1 TypeScript prompt builders for all 5 agents, callable from n8n"
  artifacts:
    ...
    - path: "src/components/campaign/generation-progress.tsx"
      ...
      contains: "agentSteps"
    - path: "src/app/api/internal/build-prompt/route.ts"
      provides: "Prompt serialization endpoint for n8n agent sub-workflows"
      exports: ["POST"]
  key_links:
    ...
    - from: "src/app/api/webhooks/n8n/route.ts"
      ...
      pattern: "agentSteps"
    - from: "(n8n) Agent sub-workflows"
      to: "src/app/api/internal/build-prompt/route.ts"
      via: "HTTP Request with HMAC"
      pattern: "buildSystemPrompt.*buildUserMessage.*getToolSchema"
---
...
<task type="auto">
  <name>Task 4: Create /api/internal/build-prompt endpoint (single source of truth for agent prompts)</name>
  <files>
    src/app/api/internal/build-prompt/route.ts
  </files>
  <action>
Create an internal API endpoint that serializes agent prompts from the Phase 9.1 TypeScript prompt builders. This is the single source of truth for all agent prompts — n8n sub-workflows call this endpoint instead of inlining prompt content. This prevents prompt drift between the TypeScript builders and n8n Code nodes.

**Security:** Verify HMAC signature using N8N_WEBHOOK_SECRET (same pattern as other internal endpoints / callback handler).

**Request body:**
```typescript
interface BuildPromptRequest {
  agentName: "strategicInsight" | "creativeDirector" | "copywriter" | "artDirector" | "jpLocalization"
  brief: CampaignBrief
  ...
  // JP Localization revision support
  revisionContext?: {
...
**Routing logic by agentName:**
- `strategicInsight`: Call `buildStrategicInsightSystemPrompt()`, `buildStrategicInsightUserMessage(brief, brandProfile)`, return `STRATEGIC_INSIGHT_TOOL_SCHEMA`. Temperature 0.2, maxTokens 4096.
- `creativeDirector`: Call `buildCreativeDirectorSystemPrompt()`, `buildCreativeDirectorUserMessage(brief, brandProfile, upstreamOutputs.strategicInsight)`, return `getCreativeDirectorToolSchema(mode)`. Temperature 0.5, maxTokens 4096.
- `copywriter`: Call `buildCopywriterSystemPrompt(brief.campaignProductInfo.category)`, `buildCopywriterUserMessage(brief, brandProfile, upstreamOutputs.strategicInsight, upstreamOutputs.creativeDirector)`, return `COPYWRITER_TOOL_SCHEMA`. Temperature 0.7, maxTokens 8192.
- `artDirector`: Call `buildArtDirectorSystemPrompt()`, `buildArtDirectorUserMessage(brief, brandProfile, upstreamOutputs.strategicInsight, upstreamOutputs.creativeDirector)`, return `getArtDirectorToolSchema(mode)`. Temperature 0.5, maxTokens 8192.
- `jpLocalization`: Call `buildJpLocalizationSystemPrompt()`, either `buildJpLocalizationUserMessage(...)` or `buildJpLocalizationRevisionMessage(...)` depending on revisionContext presence, return `JP_LOCALIZATION_TOOL_SCHEMA`. Temperature 0.2, maxTokens 4096.

**Error handling:**
- Return 401 for missing/invalid HMAC signature
- Return 400 for unknown agentName or missing required fields
- Return 200 with serialized prompt payload on success

**Why this matters:** The Phase 9.1 prompt builders are the single source of truth for agent prompts. They contain embedded framework knowledge (Schwartz levels, LF8 desires, copywriting frameworks, register maps, Flux techniques, quality checklists). Inlining this content in n8n Code nodes would create drift — any Phase 9.1 builder update would not reach n8n workflows. This endpoint bridges the gap.
  </action>
  <verify>
    - `npx tsc --noEmit` passes
    - Endpoint file exists at src/app/api/internal/build-prompt/route.ts
    - Endpoint imports from all 5 agent prompt builders in src/lib/ai/prompts/agents/
    - Endpoint verifies HMAC signature
    - Endpoint returns serialized {systemPrompt, userMessage, toolSchema, toolChoice, temperature, maxTokens} for each agent
    - `grep -r "buildStrategicInsightSystemPrompt\|buildCreativeDirectorSystemPrompt\|buildCopywriterSystemPrompt\|buildArtDirectorSystemPrompt\|buildJpLocalizationSystemPrompt" src/app/api/internal/build-prompt/route.ts` returns matches for all 5
  </verify>
  <done>
    - /api/internal/build-prompt endpoint exists and handles all 5 agents
    - Each agent routes to its Phase 9.1 TypeScript prompt builder (single source of truth)
    - Returns serialized prompt payload ready for n8n HTTP Request to Anthropic API
    - HMAC-secured for internal-only access
    - JP Localization revision context supported for critique loop
  </done>
</task>
...
- /api/internal/build-prompt endpoint returns serialized prompts for all 5 agents
</verification>
...
- /api/internal/build-prompt endpoint serves as single source of truth for all 5 agent prompts (prevents drift from Phase 9.1 builders)
</success_criteria>
