...
**GENX-07 (ZIP packaging):**
ZIP packaging already exists at `src/lib/platforms/zip-packager.ts` and is generated on-demand when the user clicks "Download". The existing download endpoint (or campaign detail page) calls `buildCampaignZip()`. No new n8n workflow needed. Verify the endpoint exists and works by checking for a download route in the campaign pages.
...
Already implemented in `src/lib/ai/provider-health.ts` — 3 consecutive failures open the circuit, 5-minute cooldown. Used automatically by `runVideoPipeline()` which checks provider health before each call.
  ...
  <verify>
    ...
    - ZIP packager endpoint/function exists (verify with `grep -r "buildCampaignZip"`)
  </verify>
  <done>
    ...
    - ZIP download works with all campaign assets
  </done>
</task>
...
- ZIP download includes all campaign assets
